  ...
  tabPanel("Coverage Analysis",
           ...
           sidebarPanel(
             ...
             pickerInput("coverage_co",
                         ...
                         choices = c(1:1000, "all",names("file1")), options = list(`actions-box` = TRUE), multiple =FALSE),
             ...
             pickerInput("exon_number",
                         label= "exon number",
                         choices = c(1:150),options = list(`actions-box` = TRUE), multiple =FALSE),
           ...
           mainPanel(
             ...
             shinyBS::bsButton("example_data",label= "load example dataset",  icon = icon("file"), style = "info", size = "extra-small"),
             helpText(em("load example dataset with base coverage counts of POLG gene")),
             hr(),

             tabsetPanel(
               tabPanel("bed file", DT::dataTableOutput("text")),
               tabPanel("UCSC gene", DT::dataTableOutput("ccg")),
               tabPanel("UCSC exons", helpText(em("Extract protein coding positions from UCSC")), DT::dataTableOutput("exon_pos")),
               tabPanel("Low-coverage positions", DT::dataTableOutput("text_cv")),
               tabPanel("Gene coverage", plotOutput("all_gene"), DT::dataTableOutput('df.l')),
               tabPanel("Exon coverage", helpText(em("This is a Gviz function and it plots exon with ideogram, genome coordinates, coverage information, Ensembl and UCSC gene annotation.
                        ...
                        plotOutput("ens", dblclick = "plot1_dblclick",  brush = brushOpts( id = "plot1_brush", resetOnNew = TRUE)), DT::dataTableOutput("tabExon")),
